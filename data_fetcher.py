...
def ambil_harga_realtime(ticker):
    """Mengambil harga saham BEI real-time dengan Fast-Fail Fallback."""
    
    # LAYER 1: Google Finance Scraper (Prioritas Real-Time)
    user_agents = [
    ...
    try:
        # TIMEOUT EKSTREM: 1.5 detik. Jika Google tidak merespons cepat, langsung ke Layer 2.
        response = requests.get(url, headers={'User-Agent': random.choice(user_agents)}, timeout=1.5)
        ...
        pass # Drop koneksi tanpa error log, pindah rute ke Layer 2
        
    # LAYER 2: YFinance API (Jalur Backup Stabil & Cepat)
    try:
