...
def render_tab_saham_deviden_diskon(daftar_pantauan):
    ...
    if top_diskon:
        ...
        for i in range(0, len(top_diskon), MAX_COLS):
            ...
            # Selalu gunakan MAX_COLS tetap agar lebar kartu konsisten di semua baris
            ...
            for j, data in enumerate(baris_saham):
                with cols_diskon[j]:
                    ...
                    # HTML diratakan ke kiri penuh agar Streamlit/Markdown tidak 
                    # sengaja mengonversinya menjadi Code Block (kotak putih teks kode).
                    st.markdown(f"""
<div style="background-color: #1e1e1e; padding: 15px; border-radius: 10px; border-left: 5px solid #ef4444; border-right: 1px solid #333; border-top: 1px solid #333; border-bottom: 1px solid #333; margin-bottom: 15px; box-shadow: 2px 2px 8px rgba(0,0,0,0.3); min-height: 260px; display: flex; flex-direction: column; justify-content: space-between;">
<div>
