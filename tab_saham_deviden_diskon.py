...
def render_tab_saham_deviden_diskon(daftar_pantauan):
    ...
    if top_diskon:
        ...
        # GRID SYSTEM: Maksimal 5 kolom per baris
        MAX_COLS = 5 
        for i in range(0, len(top_diskon), MAX_COLS):
            ...
            for j, data in enumerate(baris_saham):
                with cols_diskon[j]:
                    ...
                    # Kartu UI HTML/CSS
                    st.markdown(f"""
                    <div style="background-color: #1e1e1e; padding: 15px; border-radius: 10px; border-left: 5px solid #ef4444; border-right: 1px solid #333; border-top: 1px solid #333; border-bottom: 1px solid #333; margin-bottom: 15px; box-shadow: 2px 2px 8px rgba(0,0,0,0.3);">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <h2 style="margin:0; color: #f87171; font-family: monospace;">{data['ticker']}</h2>
                        </div>
                        <h1 style="margin: 5px 0; color: white; font-size: 1.6rem;">▼ {data['penurunan_persen']:.2f}%</h1>
                        
                        <div style="background-color: #2d2d2d; padding: 8px; border-radius: 6px; margin: 10px 0;">
                            <p style="margin:0; color: #e5e7eb; font-size: 0.85rem;">Harga Skrg : <b style="color:white;">Rp {data['harga_sekarang']:,.0f}</b></p>
                            <p style="margin:0; color: #9ca3af; font-size: 0.8rem;">1 Bln Lalu : <del>Rp {data.get('harga_sebulan_lalu', 0):,.0f}</del></p>
                        </div>
                        
                        <div style="margin: 10px 0;">
                            <p style="margin:0; color: #10b981; font-size: 0.85rem;" title="Tertinggi 1 Bulan">🔼 Res : Rp {data['resistan']:,.0f}</p>
                        ...
                        <p style="margin:0; color: #9ca3af; font-size: 0.75rem;">Bandarmologi (Estimasi 1B):</p>
                        <p style="margin:0; color: {warna_bandar}; font-size: 0.85rem; font-weight: 600;">
                            {ikon_bandar} {status_bandar} (Rp {data.get('dana_masuk_milyar', 0):,.1f} M)
                        </p>
                    </div>
